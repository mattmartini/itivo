...
# PSP
   ...
   $width=480;
   $height=272;
   $shellScript3 = $file2 . "Contents/Resources/mencoder -af volume=13:1 -of lavf -lavfopts format=psp -oac faac -faacopts mpeg=4:object=2:raw:br=$abitrate -ovc x264 -x264encopts nocabac:level_idc=21:bitrate=$vbitrate:threads=auto:bframes=0:global_header:subq=5:me=umh -vf pp=lb,scale=$width:$height,harddup -o " . $file3 . $file . $filenameExtension ." ~/.TiVoDLPipe2";
} elsif ($encodeMode == 7) {
# PS3
   $vbitrate=960;
   $abitrate=128;
